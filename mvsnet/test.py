...
tf.app.flags.DEFINE_integer('max_h', 448, 
        ...
        while True:
            for data in self.sample_list: 
                ...
                for view in range(min(self.view_num, selected_view_num)):
                    image_file = file_io.FileIO(data[2 * view], mode='rb')  # use binary mode!
...
def mvsnet_pipeline(mvs_list):
    ...
    with tf.Session(config=config) as sess:   
        ...
        for step in range(len(mvs_list)):
            ...
            out_ref_image_path = os.path.join(output_folder, '%08d.jpg' % out_index)
            out_ref_image = np.squeeze(out_ref_image)
            out_ref_image = (np.clip(out_ref_image, 0, 1) * 255).astype(np.uint8)
            imageio.imwrite(out_ref_image_path, out_ref_image)

            # Convert float image to uint8 if needed
            out_ref_image = np.squeeze(out_ref_image)
            if out_ref_image.dtype == np.float32 or out_ref_image.max() <= 1.0:
                out_ref_image = (np.clip(out_ref_image, 0, 1) * 255).astype(np.uint8)
